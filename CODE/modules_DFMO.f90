...
module popolazione
	...
	integer		   :: nelem 
	!----------------------------------------------------------------
	! la popolazione e' un array la cui generica riga di dimensione n+q+n+2 
	! e' intesa come segue:
	!
	!   ------------------------------------------------------------------------------------
	...
	!   ------------------------------------------------------------------------------------
	! 
	! npop indica il numero di righe nella matrice
	! nelem indica il numero totale di righe utilizzate
	! flag_busy = 100 se la riga corrispondente e' utilizzata
	!           = 0   se la riga corrispondente e' vuota/libera
	!
	!----------------------------------------------------------------
	real*8, allocatable :: Lpop(:,:)

	contains

	subroutine insert_point(n,q,x,f,alfa,alfac)
		!--------------------------------------------------------------
		! inserisce il punto passato dentro Lpop in posizione nelem+1
		! N.B. incrementa nelem
		!--------------------------------------------------------------
		implicit none
		integer n,q
		real*8 x(n), f(q), alfac(n), alfa

		if(nelem < npop) then
			nelem = nelem + 1
			Lpop(nelem,      1:n      ) = x
			Lpop(nelem,n    +1:n+q    ) = f
			Lpop(nelem,n+q+1          ) = 100.d0
			Lpop(nelem,n+q+2          ) = alfa
			Lpop(nelem,n+q+2+1:n+q+2+n) = alfac
		endif
		return
	end subroutine insert_point

end module popolazione
