...
!============================================================================================
subroutine sd_box(n,q,bl,bu,alfa_stop,nf_max,nf,iprint,istop,hschoice)
	use vincoli
	use eps_mod
	use cache
	use filtro
	...
	real*8 :: bl(n),bu(n),alfa_stop,maxeps , fbest
	...
	write(*,*) nf, num_gen, Lmaxalfa, Lmaxalfa2
	...
	do pnum = 1,ndim
		...
		if ( condizione ) then
			...
			!write(*,*)'EVOLVE POP ?',evolve_pop
			!write(*,*)'ANALIZZA PUNTO ', pn
			!============================
...
100        format('nf=',i5,'   alfamax=',d12.5)
			...
			do ii = 1,n
				d = d_coord(ii,:)
				!write(*,*) 'uso la ',ii,'direzione coord'
			   ...
			   DO IELLE = 1,2
				...
				!write(55,*) d
				!write(*,*) 'ENTRO NELLA LINESEARCH COORD CON alfa=',alfa_d(ii),nf
				...
				if(Lkappa_domina(n,q,fz,gamma*alfa*alfa)) then 
					...
					call linesearchbox_cont(n,q,x,f,d,alfa,alfa_d,alfa_dense,z,fz,ii,&
						  ...
						  ! pause
					!write(*,*) 'ESCO  DALLA LINESEARCH COORD CON alfa=',alfa_d(ii),nf
			...
			do ii = 1,n
				...
				d_dense = H(:,ii)
				!write(*,*) 'uso la',ii,' direzione densa'

			    DO IELLE = 1,2
				!write(55,*) d_dense
				!write(*,*) 'ENTRO NELLA LINESEARCH DENSE CON alfa=',alfa_dense,nf
				!pause
				...
				if(Lkappa_domina(n,q,fz,gamma*alfa_dense*alfa_dense).or.(dsqrt(dot_product(z-x,z-x)).le.1.d-8)) then 
					...
					call linesearchbox_dense(n,q,x,f,d_dense,alfa,alfa_dense,alfa_d,z,fz,&
						  ...
						  ! pause
					!write(*,*) 'ESCO  DALLA LINESEARCH DENSE CON alfa=',alfa,nf
	...
	if(cambio_eps) then
		!write(*,*) 'recompute the NEW PARETO FRONT Lkappa with new EPS'
		!write(*,*) 'and re-initialize alfa_dense and alfa_coord for every point'
		do pn = 1,ndim
			if(Lkappa(pn,n+q+1) > 50.d0) then
				x = Lkappa(pn,1:n)
				if(.not.find_in_cache(n,q,m,x,fob,ciq)) then   
				...
				call functs_pen(n, q, m, x, f, fob, ciq)
				Lkappa(pn,n+1:n+q) = f
				Lkappa(pn,n+q+2) = alfainiz
				Lkappa(pn,n+q+2+1:n+q+2+n) = alfaciniz
			endif
		enddo
		Ltilde = Lkappa
		Lkappa(:,n+q+1) = 0.d0
		do pn = 1,ndim
			if(Ltilde(pn,n+q+1) > 50.d0) then
				call insert_row(n,q,ndim,Ltilde(pn,:),Lkappa,flag_change)
			endif
		enddo
	endif
	...
	if (.not.trovato) then
      ...
      write(*,*) 'Numero di Generazioni = ',num_gen
      write(*,*) 'n.f.evals(inc. cache hits) = ',ncache,' n.f.evals(eff) = ',nf

700   format(I7,' & ',I7,' & ',I7,' & ',I7,' & ',I7,' \\\hline')
      return
